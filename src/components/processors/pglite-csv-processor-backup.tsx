...
export function PgliteCsvProcessor({
  ...
  const getTableNameFromUrl = useCallback((csvUrl: string): string => {
    try {
      const urlObj = new URL(csvUrl, window.location.origin); // Provide base for relative URLs like /api/...
  ...
  const processUrl = useCallback(
    async (csvUrl: string) => {
      ...
      setState({ status: "fetching", progress: "Initiating CSV processing...", startTime, error: undefined, data: undefined, warnings: [] });
      ...
      try {
        ...
        if ((tableExistsRes.rows[0] as any)?.exists) {
          const rowCountRes = await db.query(`SELECT COUNT(*) as count FROM "${tableName}";`);
          if (Number((rowCountRes.rows[0] as any)?.count) > 0) {
             setState(prev => ({ ...prev, status: "completed", progress: `Using existing table '${tableName}'.`, processingTime: Date.now() - startTime, warnings: [`Using existing table: ${tableName}`]}) );
        ...
        setState(prev => ({ ...prev, status: "fetching", progress: "Downloading CSV file..." }));
        const response = await fetch(csvUrl, { signal, headers: { 'Accept-Encoding': 'gzip' } });
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        if (!response.body) throw new Error("Response body is empty.");

        const stream = response.body;

        setState(prev => ({ ...prev, status: "parsing", progress: "Parsing CSV data..." }));

        // Convert ReadableStream to text for Papa Parse
        ...
        const csvText = textDecoder.decode(combinedArray);
          ...
          Papa.parse(csvText, {
            ...
            step: async (results, parser) => {
              ...
              if (results.errors.length > 0) {
                const newWarnings = results.errors.map(e => `Row ${e.row}: ${e.message} (Code: ${e.code})`);
                setState(prev => ({ ...prev, warnings: [...(prev.warnings || []), ...newWarnings] }));
              }

              const rowData = results.data as Record<string, any>;

              if (headers.length === 0 && results.meta.fields) {
                originalFields = results.meta.fields;
                let sanitizedHeaders = originalFields.map(h => 
                  (h || `col_${originalFields.indexOf(h)}`) // Handle null/empty original headers
                  .replace(/[^a-zA-Z0-9_]/g, "_") // Sanitize
                  .replace(/^[^a-zA-Z_]+|[^a-zA-Z0-9_]+$/g, "") // Trim invalid leading/trailing chars
                  .substring(0, 63) // Max identifier length in Postgres
                );

                // Ensure no empty headers after sanitization and deduplicate
                const headerCounts: Record<string, number> = {};
                headers = sanitizedHeaders.map((h, i) => {
                    let currentHeader = h || `col_${i}`; // Fallback if sanitization results in empty string
                    headerCounts[currentHeader] = (headerCounts[currentHeader] || 0) + 1;
                    return headerCounts[currentHeader] > 1 ? `${currentHeader}_${headerCounts[currentHeader] -1}` : currentHeader;
                });
                
                setState(prev => ({ ...prev, status: "inserting", progress: `Creating table '${tableName}'...` }));
              ...
              if (headers.length > 0) {
                ...
                if (batch.length >= batchSize) {
                  parser.pause();
                  try {
                    setState(prev => ({ ...prev, progress: `Inserting batch into '${tableName}' (rows ${rowCount - batch.length + 1}-${rowCount})...` }));
                    await db.transaction(async (tx) => {
                      for (const record of batch) {
                        const values = originalFields.map(origField => record[origField]);
                        const placeholders = headers.map((_, i) => `$${i + 1}`).join(", ");
                        await tx.query(`INSERT INTO "${tableName}" (${headers.map(h => `"${h}"`).join(", ")}) VALUES (${placeholders});`, values);
                      }
                    });
                    batch = [];
                  } catch (e) {
            ...
            complete: async () => {
              ...
              try {
                if (batch.length > 0 && headers.length > 0) {
                  setState(prev => ({ ...prev, progress: `Inserting final batch into '${tableName}' (rows ${rowCount - batch.length + 1}-${rowCount})...` }));
                  await db.transaction(async (tx) => {
                     for (const record of batch) {
                        const values = originalFields.map(origField => record[origField]);
                        const placeholders = headers.map((_, i) => `$${i + 1}`).join(", ");
                ...
                if (headers.length === 0 && rowCount === 0 && !signal.aborted) {
                  throw new Error("CSV parsing yielded no data or headers. The file might be empty or in an unexpected format.");
                }
                resolve();
              } catch (e) {
                reject(e);
        ...
        if (!signal.aborted) {
            setState(prev => ({ ...prev, status: "completed", progress: `Successfully processed and loaded ${rowCount} rows into '${tableName}'.`, processingTime }));
            const initialData = await fetchTableData(tableName, { limit: tableState.pageSize });
            if (initialData) setState(prev => ({ ...prev, data: initialData }));
        }
        ...
        if (error.name === 'AbortError') {
          ...
          setState(prev => ({ ...prev, status: "idle", progress: "Processing aborted.", error: undefined, currentProcessingTime: undefined, startTime: undefined }));
        } else {
          console.error("Error processing CSV with PGLite:", error);
          setState(prev => ({ ...prev, status: "error", error: error.message, progress: "", currentProcessingTime: undefined, startTime: undefined }));
        }
      } finally {
  ...
  const clearCurrentTable = useCallback(async () => {
    ...
    try {
      ...
      setState(prev => ({ ...prev, status: "error", error: `Failed to clear table: ${err.message}` }));
    }
  ...
  const clearDatabaseAndReinitialize = useCallback(async () => {
    ...
    try {
      ...
      setState(prev => ({ ...prev, status: "error", error: `Failed to reinitialize database: ${err.message}` }));
    }
  }, [reinitializeInstance, dataDir]);
